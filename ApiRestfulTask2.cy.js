Cypress.on('uncaught:exception', (err, runnable) =>
   ...
   describe('RestFul Booking Site API Testing', ()=> 
   {
       ...
       .then(function(response){
              ...
              expect(response.body.booking.firstname).equal("Rajesh")
              expect(response.body.booking.lastname).equal("kannan")
   ...
   .then((response) =>{
    ...
    .then((response)=>{
           expect(response.status).to.eq(200)
           expect(response.body.firstname).equal("rajesh")
           expect(response.body.lastname).equal("kannan")
   ...
   {
          expect(response.status).equal(200)
          expect(response.body.firstname).equal('qaoncloud')
          expect(response.body.lastname).equal('madurai')
